...
/** Potência CC (kWp) — prioriza potenciaCc da proposta; evita campo genérico `potencia` (pode ser geração) */
function extrairPotenciaCc(raw) {
  const direto = pickNumeroPositivo(
    raw.potenciaCc,
    raw.PotenciaCC,
    raw.PotenciaCc,
    raw.potenciaCC,
    ...
    raw.Kwp,
    raw.KWP,
    raw.potenciaInstaladaCc,
    raw.PotenciaInstaladaCc,
    raw.potenciaInstalada,
    raw.PotenciaInstalada,
    raw.potenciaSistema,
    raw.PotenciaSistema,
    raw.potenciaModulos,
  ...
  if (direto) return arredondarPotencia(direto);
  ...
  for (const obj of objetosAninhados) {
    if (!obj || typeof obj !== 'object') continue;
    const valor = extrairPotenciaCc(obj);
    if (valor) return valor;
  }
  ...
  for (const lista of listasKit) {
    ...
    for (const kit of lista) {
      if (!kit || typeof kit !== 'object') continue;
      const valor = extrairPotenciaCc(kit);
      if (valor) return valor;
    }
  }
...
function arredondarPotencia(valor) {
...
export function mapPrePropostaValores(raw) {
  ...
  const potenciaKwp = extrairPotenciaCc(raw);
...
export function mapProjetoDetalhe(p) {
  ...
  const potencia = extrairPotenciaCc(p);
...
export function montarPayloadFormulario(lead, projeto) {
  ...
  const veioDaProposta =
    projeto?.propostaFonte === 'aceita' || projeto?.propostaFonte === 'ultima';
  const kwp = veioDaProposta
    ? projeto?.potenciaKwp ?? undefined
    : projeto?.potenciaKwp ?? consumoParaKwp(consumo) ?? undefined;
